...
namespace PaintIn3D
{
	...
	public class P3dInputManager : MonoBehaviour
	{
		...
		protected virtual void Update()
		{
			...
			{
				Debug.DrawRay(maleGO.transform.position, transform.TransformDirection(Vector3.left) * hit.distance, Color.yellow);
				WallPaint(hit.point);
			}*/

				if (Physics.Raycast(maleGO.transform.position, Vector3.left, out var hit, 1f, layerMask))
				{
					Debug.DrawRay(maleGO.transform.position, transform.TransformDirection(Vector3.left) * hit.distance, Color.yellow);
				...
				if (Physics.Raycast(femaleGO.transform.position, Vector3.right, out var hit2, 2f, layerMask))
				{
					Debug.DrawRay(femaleGO.transform.position, transform.TransformDirection(Vector3.right) * hit2.distance, Color.yellow);
					//print("Found an object - distance: " + hit2.distance);
					WallPaint(hit2.point);
				}
			

			/*if (isFind && Physics.Raycast(motorGO.transform.position, Vector3.left, out var hit3, 3))
			{
				Debug.DrawRay(motorGO.transform.position, transform.TransformDirection(Vector3.left) * hit3.distance, Color.yellow);
				print("Found an object - distance: " + hit3.distance);
				WallPaint(hit3.point);
			}*/
		}

		public void WallPaint(Vector3 hitPoint)
		{
			//Vector3 newVector3 = new Vector3(hitPoint.z, hitPoint.y, hitPoint.x);
			var p = Camera.main.WorldToScreenPoint(hitPoint);


			AddFinger(1337, p, 1.0f, false);
			//print(hitPoint + "" +p);
		}
